export class Utils {
	...
	static diferencaDeArrays<T>(array1: T[], array2: T[]): T[] {
		const arr1TemObj = array1.some((el)=>typeof el === "object")
		const arr2TemObj = array2.some((el)=>typeof el === "object")
		if (arr1TemObj || arr2TemObj){
			throw new Error("Não pode ter objetos nos arrays")
		}
	...
	static removerDuplicados<T>(array: T[]): T[] {
		...
		if (arrTemObj){
			throw new Error("Não pode ter objetos no array")
		}
